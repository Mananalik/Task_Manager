...
import User from '../../models/auth/userModule.js';   
...
export const loginUser = asyncHandler(async(req,res)=>{
  ...
  const token = generateToken(userExists._id);
   if(userExists && isMatch){
    ...
    // send token back to the user in response
    res.status(200).json({
      _id,
      name,
      email,
...
export const verifyEmail = asyncHandler(async(req,res)=>{
  ...
  // check if user is always verified
  if(user.isVerified){
...
try{
  ...
  console.log("Error sending email: ", error);
  return res.status(500).json({message:"Emaill could not be send"});
}
});
...
export const resetPassword = asyncHandler(async(req,res)=>{
  ...
  // find the user with the user Id in the token
  const user = await User.findById(userToken.userId);
